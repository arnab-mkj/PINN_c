...
double schrodinger_equation_loss(double psi, double psi_target, double potential, double time_step){
    ...
    double loss = pow(difference, 2) + pow(kinetic_energy, 2) + pow(potential_energy, 2);
    loss /= adaptive_normalization(&loss, 1); // Normalize after computing loss
...
double maxwell_equations_loss(double electric_field, double magnetic_field, double charge_density, double current_density) {
    ...
    double gauss_loss = pow(gauss_residual * EPSILON_0, 2); // Scale to avoid large values
    double ampere_loss = pow(ampere_residual / MU_0, 2); // Scale to avoid small values

    // Total loss calculation
    double total_loss = (gauss_loss + ampere_loss);

    double losses[2] = {gauss_loss, ampere_loss};
    total_loss /= adaptive_normalization(losses, 2);
...
double heat_equation_loss(double u, double u_target, double dx, double dt){
    ...
    // double thermal_diffusivity = 1.0; // Simplified for testing; adjust as needed
    // double time_derivative = (u - u_target) / dt;
    // double second_spatial_derivative = (u - u_target) / (dx * dx); // Placeholder

    // double loss = pow(thermal_diffusivity * second_spatial_derivative - time_derivative, 2) / (adaptive_normalization(&loss, 1));
    double loss = pow(thermal_diffusivity * second_spatial_derivative - time_derivative, 2);
    loss /= adaptive_normalization(&loss, 1);
    return loss;
}
...
double wave_equation_loss(double u, double u_target, double time, double dx, double dt){
    ...
    double loss = pow((WAVE_SPEED * WAVE_SPEED) * spatial_term - temporal_term, 2); // Adjusted scaling
    loss /= adaptive_normalization(&loss, 1);
    return loss;
}
